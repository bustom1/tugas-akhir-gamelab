    ...
    <div class="container">
        ...
        <form action="{{ route('mata-pelajaran.update', $mata_pelajaran->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="id_jurusan" class="form-control" required>
                    @foreach($jurusans as $jurusan)
                        <option value="{{ $jurusan->id }}" {{ $mata_pelajaran->id_jurusan == $jurusan->id ? 'selected' : '' }}>
                            {{ $jurusan->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="deskripsi" class="form-control" rows="3" required>{{ $mata_pelajaran->deskripsi }}</textarea>
            </div>
